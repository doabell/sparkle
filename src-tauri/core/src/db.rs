...
pub fn init_db(path: &std::path::Path) -> Result<(Connection, bool)> {
    let conn = open_connection(path)?;
    initialize_connection(conn)
}

/// Initializes or migrates a connection while preserving its connection settings.
pub fn initialize_connection(conn: Connection) -> Result<(Connection, bool)> {
